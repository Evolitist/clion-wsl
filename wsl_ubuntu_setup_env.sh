...
echo "%sudo ALL=(ALL) NOPASSWD: /sbin/ldconfig, /usr/sbin/service ssh --full-restart, /etc/init.d/dbus start, /etc/init.d/avahi-daemon start" | sudo tee -a $SUDOERS_FILE
cat << 'EOF' >> ~/.bashrc
